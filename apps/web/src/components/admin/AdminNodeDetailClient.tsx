...
export function AdminNodeDetailClient({ nodeId }: { nodeId: string }) {
  ...
  if (error || !node)
    return <InlineAlert tone="error">{error || "Node detail is unavailable."}</InlineAlert>;
      ...
      <Panel className="animate-fade-up overflow-hidden">
        <div className="relative bg-gradient-to-br from-command/5 via-transparent to-[var(--mesh)]/5 px-7 py-8">
          ...
          <div className="flex flex-col justify-between gap-5 md:flex-row md:items-start">
            <div>
              ...
              <h2 className="mt-2 font-mono text-4xl font-black tracking-tight text-slate-950">
                {node.nodeId}
              </h2>
        ...
        <div className="grid gap-3 border-t border-slate-100 bg-slate-50/30 px-7 py-5 md:grid-cols-4">
          <InfoCard
            ...
            label="Last seen"
            value={formatJakartaTime(node.lastSeenAt)}
          />
          <InfoCard
            icon={<Broadcast size={18} weight="bold" />}
            label="Last message"
            value={formatJakartaTime(node.lastMessageAt)}
          />
          <InfoCard
            icon={<WifiHigh size={18} weight="bold" />}
            label="Range to gateway"
            value={
              node.lastRangeToGateway === null ? "Unavailable" : String(node.lastRangeToGateway)
            }
            mono
          />
          <InfoCard
            ...
            label="Location"
            value={
              node.lastKnownLat === null || node.lastKnownLon === null
                ? "Unavailable"
                : `${node.lastKnownLat.toFixed(4)}, ${node.lastKnownLon.toFixed(4)}`
            }
            mono
          />
        </div>
