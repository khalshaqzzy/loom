    ...
    <section className="landing-stage landing-hero">
      ...
      <div className="landing-nav">
        ...
        <div className="landing-nav-actions">
          ...
          <LandingButton href="/admin/login" variant="light" icon={<UserCircle size={22} weight="bold" />}>
            Admin login
          </LandingButton>
        </div>
      </div>
      ...
      <div className="hero-copy">
        ...
        <p>
          Offline field nodes connect via LoRa mesh and relay messages from device to device. When a gateway reconnects,
          data is synced to the backend and becomes visible on the public map and admin dashboard.
        </p>
        <div className="hero-actions">
          <LandingButton href="/public" icon={<MapTrifold size={23} weight="bold" />}>
            Open public map
          </LandingButton>
          <LandingButton href="/admin/login" variant="light" icon={<UserCircle size={23} weight="bold" />}>
            Admin login
          </LandingButton>
  ...
  return (
    <section className="landing-stage section-preview">
      ...
      <div className="preview-copy">
        ...
        <p>LOOM connects devices, gateways, and people when the internet is down. Explore the core experiences for public users and admins.</p>
      </div>
      ...
      <PreviewCard className="preview-map-card">
        ...
        <Image src={asset("landing/section2/preview-map-bento.png")} alt="" width={1672} height={941} className="preview-map-img" />
        <MapControls />
        <MapFilter />
        <MapStatusBar />
      </PreviewCard>
      ...
      <PreviewCard className="preview-gateway-card">
        ...
        <Image src={asset("landing/section2/preview-gateway-path.png")} alt="" width={1448} height={864} className="full-img" />
      </PreviewCard>
      ...
      <PreviewCard className="preview-privacy-card">
        ...
        <Image src={asset("landing/section2/preview-privacy-phone.png")} alt="" width={941} height={724} className="privacy-phone-img" />
      ...
      <PreviewCard className="preview-node-card">
        ...
        <Image src={asset("landing/section2/preview-node-device.png")} alt="" width={1122} height={1102} className="node-device-img" />
        <NodeRegisterMini />
      </PreviewCard>
  ...
  return (
    <section id="how" className="landing-stage section-mesh">
      <div className="mesh-left">
        ...
        <p>LOOM keeps messages moving when the internet is down, using nearby nodes. When a gateway comes online, everything syncs to the map.</p>
        <Image
        ...
        <div className="secure-callout">
          ...
          <div>
            ...
            <span>End-to-end encryption, device identity verification, and privacy-first sync.</span>
          </div>
        </div>
      </div>
  ...
  return (
    <section id="operations" className="landing-stage section-surface">
      ...
      <p className="surface-intro">
        LOOM provides two tailored experiences. Explore real-time network activity on the public map or sign in to
        manage infrastructure and messages as an administrator.
      </p>
      <div className="public-surface-card">
        ...
        <p>View real-time activity, check gateway status, and look up message history without creating an account.</p>
        <div className="public-feature-list">
          <FeatureLine icon={<WifiHigh size={27} weight="duotone" />} title="Live heatmap" copy="See where messages are flowing." />
          <FeatureLine icon={<MapTrifold size={27} weight="duotone" />} title="Marker-only view" copy="Focus on key locations." toggle />
          <FeatureLine icon={<ShieldCheck size={27} weight="duotone" />} title="Privacy lookup" copy="Check messages without revealing identities." />
        </div>
        <div className="surface-actions">
          <LandingButton href="/public" icon={<MapTrifold size={22} weight="bold" />}>Explore public map</LandingButton>
          <LandingButton href="/public/history" variant="light" icon={<ClockCounterClockwise size={22} weight="bold" />}>Lookup history</LandingButton>
        </div>
        <Image src={asset("landing/section2/preview-map-bento.png")} alt="" width={1672} height={941} className="surface-map-img" />
        <MapControls compact />
        <MapFilter compact />
      ...
      <div className="admin-surface-card">
        ...
        <AdminRow icon={<LockKey size={31} weight="duotone" />} title="Admin sign in" copy="Secure access to the operations console." img="surface-admin-login.png" />
        <AdminRow icon={<WifiHigh size={31} weight="duotone" />} title="Node registry" copy="Register and manage network devices." img="surface-node-registry.png" />
        <AdminRow icon={<WifiHigh size={31} weight="duotone" />} title="Full marker details" copy="Inspect gateways, status, and activity." img="surface-marker-details.png" />
        <AdminRow icon={<Database size={31} weight="duotone" />} title="Message history" copy="Review and trace message activity." img="surface-message-history.png" />
        <LandingButton href="/admin/login" className="admin-wide-button" icon={<LockKey size={22} weight="bold" />}>Sign in as admin</LandingButton>
      ...
      <div className="lifecycle-grid">
        {lifecycleCards.map((card, index) => (
          <div key={card.title} className="lifecycle-card">
            ...
            <h3>{card.title}</h3>
            <p>{card.copy}</p>
            <Image src={asset(`landing/section5/${card.image}`)} alt="" width={1100} height={1500} className="lifecycle-img" />
  ...
  return (
    <section id="privacy" className="landing-stage section-privacy">
      ...
      <p className="privacy-intro">
        LOOM is designed with privacy by default. We collect only what's necessary, never display birth dates, and keep
        identity data protected behind admin access.
      </p>
      ...
      <div className="privacy-bar">
        ...
        <div>
          ...
          <span>LOOM follows strict data minimization principles and industry-standard security practices to protect your information.</span>
        </div>
...
function FinalCtaSection() {
  return (
    <section className="landing-stage section-cta">
      <Image src={asset("landing/section7/final-cta-map.png")} alt="" fill className="cta-map-img" />
      ...
      <div className="cta-content">
        ...
        <p>
          LOOM connects devices, gateways, and people when the internet is down. Access the live map, review history,
          and manage your network with confidence.
        </p>
        <div className="cta-actions">
          <LandingButton href="/public" icon={<MapTrifold size={27} weight="bold" />}>
            Open public map <ArrowRight size={25} weight="bold" />
          </LandingButton>
          <LandingButton href="/public/history" variant="dark" icon={<ClockCounterClockwise size={27} weight="bold" />}>
            View history <ArrowRight size={25} weight="bold" />
          </LandingButton>
          <LandingButton href="/admin/login" variant="dark" icon={<UserCircle size={27} weight="bold" />}>
            Admin login
          </LandingButton>
        </div>
        <div className="cta-foot">
          <span><ShieldCheck size={26} weight="duotone" /> Privacy-safe lookup</span>
          <span><i /> Live operational visibility</span>
        </div>
      </div>
    </section>
  );
      ...
      <div className="footer-columns">
        {footerColumns.map(([title, ...items], index) => (
          <div key={title} className="footer-column">
            <h3>
              {index === 0 ? <Cube size={22} weight="duotone" /> : index === 1 ? <GlobeHemisphereWest size={22} weight="duotone" /> : index === 2 ? <ShieldCheck size={22} weight="duotone" /> : <Package size={22} weight="duotone" />}
              {title}
            </h3>
            {items.map((item) => (
              <a key={item} href={item === "Public map" ? "/public" : item === "Admin login" ? "/admin/login" : item === "Privacy lookup" ? "/public/history" : "#"}>
                {item}
              </a>
            ))}
          </div>
...
function MapStatusBar({ compact = false }: { compact?: boolean }) {
  return (
    <div className={`map-status-bar ${compact ? "map-status-bar-compact" : ""}`}>
      <span><i /> Live</span>
      <span>Nodes <strong>134</strong></span>
      <span>Gateways <strong>6</strong></span>
      <span>Messages (24h) <strong>512</strong></span>
...
function NodeRegisterMini() {
  return (
    <div className="node-register-mini">
      <label>Node ID <span>NODE-7F3A-2C91</span></label>
      <label>Owner full name <span>Alex Morgan</span></label>
      <label>Birth date <span>1988-07-23</span></label>
      <p><LockKey size={18} weight="bold" /> LOOM stores only what's necessary. Data is private and never shared.</p>
  ...
  return (
    <div className="message-stream-mini">
      ...
      <div className="stream-detail">
        <h4>Message <span>Canonical</span></h4>
        <dl>
          <dt>From</dt><dd>NODE-3B71</dd>
          <dt>Category</dt><dd>Medical</dd>
          <dt>Message</dt><dd>Need first aid kit in the north corridor.</dd>
          <dt>Time (local)</dt><dd>10:24 AM</dd>
          <dt>seqId</dt><dd>88420</dd>
        </dl>
        <p><CheckCircle size={18} weight="bold" /> Duplicate messages are ignored</p>
      </div>
    </div>
  );
}

function FeatureLine({ icon, title, copy, toggle = false }: { icon: ReactNode; title: string; copy: string; toggle?: boolean }) {
  return (
...
function AdminRow({ icon, title, copy, img }: { icon: ReactNode; title: string; copy: string; img: string }) {
  return (
